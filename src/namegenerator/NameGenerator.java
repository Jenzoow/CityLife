 ...
 * important.
 *
 * RULES: 1) +v means that next syllable must definitely start with a vocal. 2)
 ...
 * with a vocal. 4) -c means that this syllable can only be added to another
 * syllable, that ends with a consonant. So, our example: "aad +v -c" means that
 * "aad" can only be after consonant and next syllable must start with vocal.
 * Beware of creating logical mistakes, like providing only syllables ending
 * with consonants, but expecting only vocals, which will be detected and
...
public class NameGenerator {
    ...
    private boolean expectsVocal(String s) {
        if (s.substring(1).contains("+v") ? true : false) return true;
        else return false;
    }
